...
def plot_temperature(sp_df, dupl_df, show=False):
    """
    ...
    # Plot periodic line 
    plt.plot(sp_df["Recovered_Temperature"].values - sp_df["Recovered_Temperature"].values[0], sp_df["Points:1"].values,
             label="Periodic",
             color='red',
             linewidth=3)
...
if __name__=='__main__':
    ...
    sp_df = pd.read_csv("4__periodic-with-T-periodicty/line_0.csv")

    # Read all the duplicate lines
    dupl_df = []
    for i in range(100): # large value to be safe
        try:
            dupl_df.append(pd.read_csv("3__repeated-blockprofile/line_" + str(i) + ".csv"))
        except:
            break
    #end

    plot_vel(sp_df, dupl_df)
    plot_muT(sp_df, dupl_df)
    plot_pressure(sp_df, dupl_df)
    plot_temperature(sp_df, dupl_df)
    print('End')