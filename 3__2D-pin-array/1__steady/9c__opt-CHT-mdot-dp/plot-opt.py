...
def postprocessData(df, dfkey, infeasibleValue):
    ...
    for index, row in df.iterrows():
       if(abs(row[dfkey]) >= abs(infeasibleValue)):
...
def plotOF(df):
    ...
    ax1.plot(df['ITER'].values, df['  avgT'].values,
             color=color,
             linestyle='-',
             marker='o')
    ...
    ax2.plot(df['ITER'].values, df['  dp'].values,
             color=color,
             linestyle='-',
             marker='x')
...
def plotGradNorm(df):
    ...
    ax2.plot(df['ITER'].values, df['dp-gradNorm'].values,
             color=color,
             linestyle='-',
             marker='x')
    #ax2.set_ylim((100.0,300.0))
...
if __name__=='__main__':
    ...
    hist_df = postprocessData(hist_df, '  avgT', 1000)
    hist_df = postprocessData(hist_df, '  dp', 1000)
    plotOF(hist_df)
