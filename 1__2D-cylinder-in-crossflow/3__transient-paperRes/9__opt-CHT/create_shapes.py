...
for i,folder in enumerate(DSN_folders):
    ...
    fADO_configMastervtm = XMLMultiBlockDataReader(FileName=['./' + folder + '/DEFORM/chtMaster_00000.vtm'])
    # create a new 'Extract Block'
    extractBlock1 = ExtractBlock(Input=fADO_configMastervtm)
    # Properties modified on extractBlock1
    extractBlock1.BlockIndices = [6]
    # save data
    SaveData('./' + folder + '/DEFORM/shape.csv', proxy=extractBlock1)
    print(folder + ' done')
